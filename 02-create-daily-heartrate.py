...
for p in [8]:
    ...
    df_01 = df.resample(freq).apply(utils.get_nonzero_min)
    df_01.rename(columns={'heartrate': 'heartrate-min'}, inplace=True)

    df_02 = df.resample(freq).max()
    df_02.rename(columns={'heartrate': 'heartrate-max'}, inplace=True)

    df_03 = df.resample(freq).apply(utils.get_nonzero_mean)
    df_03.rename(columns={'heartrate': 'heartrate-mean'}, inplace=True)

    df_04 = df.resample(freq).apply(utils.get_nonzero_std)
    df_04.rename(columns={'heartrate': 'heartrate-std'}, inplace=True)
