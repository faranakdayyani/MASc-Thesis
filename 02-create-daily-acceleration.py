...
for p in [8]:
    ...
    df_01.rename(columns={'std': 'acceleration-sum'}, inplace=True)

    # df_02 = df.resample(freq).apply(utils.get_nonzero_daytime_ratio)
    # df_02.rename(columns={'step': 'step-ratio'}, inplace=True)
    #
    # df_03 = df.resample(freq).apply(utils.get_nonzero_mean)
    # df_03.rename(columns={'step': 'step-mean'}, inplace=True)
    #
    # df_04 = df.resample(freq).max().astype(int)
    # df_04.rename(columns={'step': 'step-max'}, inplace=True)
    #
    # df_05 = df.resample(freq).apply(utils.get_max_timestamp)
    # df_05.rename(columns={'step': 'step-max-timestamp'}, inplace=True)
